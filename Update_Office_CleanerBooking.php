<?php
 ...
 <?php
   ...
   if(isset($_GET['office_cleaning_booking_id']))
   {
     ...
     $sql    = "SELECT * FROM tb_office_cleaning_booking WHERE office_cleaning_booking_id = $office_cleaning_booking_id";
     ...
     while($getData = $result->fetch_assoc())
     {
       ...
       $nmbr_of_cleaner    = $getData['nmbr_of_cleaner'];
       $hour               = $getData['hour'];
  ...
  <?php
   ...
   if(isset($_POST['update']))
   {
     if(checkPhone())
     {
       ...
       $sql = "UPDATE tb_office_cleaning_booking SET name='$name',phone='$phone',email='$email',address='$address',city='$city',postcode='$postcode',nmbr_of_cleaner='$nmbr_of_cleaner',hour='$hour',cleaning_product='$cleaning_product',time='$time',date='$date',user_name='$user_name',entry_time='$current_datetime' WHERE office_cleaning_booking_id = $office_cleaning_booking_id";
    ...
    <head>
        ...
        <link
            ...
            <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/5.0.0-alpha1/css/bootstrap.min.css"
                ...
                <title>Update Office Cleaner Booking</title>
            </head>
            <body>
                        ...
                        <form action="" method="post" enctype="multipart/form-data" autocomplete="off">
                            ...
                            <div class="row">
                                <div class="col-12 mt-4">
                                    <div class="form-group">
                                        <label for="inputCleaningProducts" class="form-label">How Many Cleaner Experts Do You require?</label>
                                        <select ="nmbr_of_cleaner" name="nmbr_of_cleaner" class="form-select">
                                            <option selected><?php echo $nmbr_of_cleaner; ?></option>
                                            <?php while($getData = $hour_data->fetch_assoc()){ ?>
                                            <option><?php echo $getData['hour']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12 mt-4">
                                    <div class="form-group">
                                        <label for="inputCleaningProducts" class="form-label">How Many hours do you need us for?</label>
                                        <select id="hour" name="hour" class="form-select">
                                            <option selected><?php echo $hour; ?></option>
                                            <?php while($getData = $no_of_cleaner->fetch_assoc()){ ?>
                                            <option><?php echo $getData['no_of_cleaner']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <input type="submit" class="btn btn-danger mt-4" name="update" value="Update">
                        </form>
