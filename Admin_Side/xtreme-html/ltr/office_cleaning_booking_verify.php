  ...
  <?php
   $db = new Database();
     ...
     if(isset($_GET['office_cleaning_booking_id']))
     {
       ...
       $sql = "SELECT booking_status FROM tb_office_cleaning_booking WHERE booking_status = 0 AND office_cleaning_booking_id = '$office_cleaning_booking_id' LIMIT 1";

       $result = $db->link->query($sql) or die($this->link->error.__LINE__);

       if($result->num_rows == 1)
       {
         $sql = "UPDATE tb_office_cleaning_booking SET booking_status = 1 WHERE office_cleaning_booking_id = '$office_cleaning_booking_id' LIMIT 1";
         ...
         if($update)
         {
           ...
           <script type="text/javascript">
             ...
             window.location='Booking_table_office_cleaner.php';
